...
public class ResourceServerConfig extends GlobalMethodSecurityConfiguration {
	...
	SecurityFilterChain oauth2AuthFilterChain(HttpSecurity http) throws Exception {
        ...
        http.cors().disable().authorizeRequests().anyRequest().authenticated().and().addFilterAfter(new CustomTokenFilter(this.redisAdapter), BearerTokenAuthenticationFilter.class).logout()
