...
//
    ...
    @Query("select u from VUser u join VUserRole ur on u.id = ur.userId where u.status = :status and ur.role = 2 and u.numberPhone like :numberPhone")
    ...
    @Query("select u from VUser u join VUserRole ur on u.id = ur.userId where u.status = :status and ur.role = 2 and u.cccd like :cccd")
    ...
    @Query("select u from VUser u join VUserRole ur on u.id = ur.userId where u.status = :status and ur.role = 2 and u.email like :email")
    ...
    @Query("select u from VUser u join VUserRole ur on u.id = ur.userId where u.status = :status and ur.role = 2 and u.fullname like :fullname")
