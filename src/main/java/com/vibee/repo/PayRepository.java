...
public interface PayRepository extends JpaRepository<VPay, Integer>, JpaSpecificationExecutor<VPay> {
    List<VPay> findByDebitId(int debitId);

    List<VPay> existsByDebitId(int debitId);
    ...
    @Query(value = "select sum(v.NUMBER_OF_PAYOUTS) as num from v_pay v where v.DEBIT_ID=?1", nativeQuery = true)
